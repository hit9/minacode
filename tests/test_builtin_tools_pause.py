...
async def test_paused_turn_is_reported_and_replays_unchanged(tmp_path, monkeypatch):
    ...
    assistant, calls, _ = await model.request([{"role": "user", "content": "hi"}], [])
...
async def test_an_unpaused_response_carries_no_pause_marker(tmp_path, monkeypatch):
...
async def test_agent_continues_a_paused_turn_instead_of_answering(tmp_path):
    ...
    assert await agent.run("look it up") == "found it"
...
async def test_a_paused_turn_is_bounded_by_max_steps(tmp_path):
    ...
    assert "Stopped after max_agent_steps=3" in await agent.run("look it up")
...
async def test_a_declared_builtin_function_call_is_answered_with_its_arguments(tmp_path):
    ...
    messages = await runner.run([ToolCall("c1", "$web_search", [{"search_query": "httpx timeout"}])])
...
async def test_an_undeclared_builtin_function_call_is_still_an_unknown_tool(tmp_path):
    ...
    messages = await runner.run([ToolCall("c1", "$web_search", [{"search_query": "x"}])])

    assert "unknown tool $web_search" in messages[0]["content"]


async def test_a_batch_mixing_an_echo_and_a_real_tool_runs_both(tmp_path):
    s = agent_session(tmp_path)
    s.config.providers["default"].builtin_tools = ({"type": "builtin_function", "function": {"name": "$web_search"}},)
    (tmp_path / "a.txt").write_text("alpha\n", encoding="utf-8")
    runner = ToolRunner(s, ContextManager(s), input_fn=lambda *a: "", output_fn=lambda text: None)

    messages = await runner.run([ToolCall("c1", "$web_search", [{"search_query": "q"}]), ToolCall("c2", "Read", [{"path": "a.txt", "ranges": [[0, 1]]}])])
