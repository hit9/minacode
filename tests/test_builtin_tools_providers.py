...
async def test_aliyun_chat_keeps_responses_builtin_tools_inactive(tmp_path, monkeypatch):
...
async def test_qwen_responses_keeps_builtin_tools_unchanged(tmp_path, monkeypatch):
...
async def test_unknown_provider_keeps_generic_builtin_tools_pass_through(tmp_path, monkeypatch):
...
async def test_zai_and_bigmodel_chat_accept_their_documented_builtin_tool(tmp_path, monkeypatch, url):
...
async def test_kimi_chat_accepts_builtin_function_unchanged(tmp_path, monkeypatch):
...
async def test_anthropic_builtin_tools_are_protocol_scoped(tmp_path, monkeypatch):
...
async def test_openrouter_sends_supported_server_tools_unchanged_on_both_wires(tmp_path, monkeypatch, api):
...
async def test_estimation_and_send_share_the_builtin_tools_policy(tmp_path, monkeypatch):
