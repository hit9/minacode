...
class ToolRunner:
        ...
        try:
            ...
            if isinstance(tool, BashTool) and self.live_start is not None:
                if not d.nested_display:
                    self.emit(
                        LogBlock.hierarchy(self.log_root(d.display or tooloutput.short_call(self.session, call), batch_suffix=batch_suffix, call=call), [])
                    )
                    d.nested_display = True
                self.live_start()
                ...
                # not reassurance.
                self.emit(LogBlock.hierarchy(self.log_root(d.display or tooloutput.short_call(self.session, call), batch_suffix=batch_suffix, call=call), []))
                d.nested_display = True
    ...
    def reject_display(self, call: ToolCall, output: str, *, d: ToolDisplay) -> LogBlock:
        ...
        display = oneline(d.display or tooloutput.short_call(self.session, call), 120)
    ...
    def approval_display(
        ...
        root = self.log_root(tooloutput.short_call(self.session, call), role, batch_suffix, call)
        children = []
    ...
    def finish_display(
        ...
        *,
        ...
        label = d.display or tooloutput.short_call(self.session, call)
