...
* - **`NextHints`**
  - Offers 2–3 short next-step prompts the model suggests after its answer. They appear as
    ...
    input (press again to unpick) and returns to the prompt, so `Tab` to the next chip and
    ...
    `Tab` moves the highlight; `Enter` picks the focused chip into the input line (again
    unpicks) and returns to the prompt, so `Tab` to the next chip and `Enter` again combines
    several; a final `Enter` sends.
* - **`Skill`**
